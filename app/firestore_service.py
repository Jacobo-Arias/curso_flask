import firebase_admin
from firebase_admin import credentials
from firebase_admin import firestore

# las credenciales de la aplciación por defecto en fifebase
# la aplicación por defecto se establece con gcloud init
credential = credentials.ApplicationDefault()
# igualmente para inicial la aplicación default no son necesarios parametros
# o credenciales
default_app = firebase_admin.initialize_app()

db = firestore.client()

def get_users():
    return db.collection('users').get()

def get_user(user_id):
    return db.collection('users').document(user_id).get()


def user_put(user_data):
    #esto porque el "username" es el id
    user_ref = db.collection('users').document(user_data.username)
    user_ref.set({'password':user_data.password})

def get_todos(user_id):
    # esto es por la estructura de la base de datos firestore
    # collectiones>documents>collections
    return db.collection('users').document(user_id).collection('todos').get()

def put_todo(user_id, description):
    todos_collection_ref = db.collection('users').document(user_id).collection('todos')
    # añade un nuevo documento con id aleatorio
    todos_collection_ref.add({'description':description, 'done':False})

def delete_todo(user_id, todo_id):
    # estas dos formas son equivalentes
    # -> todo_ref = db.collection('users').document(user_id).collection('todos').document(todo_id)
    # todo_ref = db.document(f'users/{user_id}/todos/{todo_id}')
    # todo_ref.detele()
    todo_ref = _get_todo_ref(user_id=user_id, todo_id=todo_id)
    todo_ref.delete()

def update_todo(user_id, todo_id, done):
    todo_done = not bool(done)
    todo_ref = _get_todo_ref(user_id=user_id, todo_id=todo_id)
    todo_ref.update({'done':todo_done})

def _get_todo_ref(user_id,todo_id):
    return db.document('users/{}/todos/{}'.format(user_id, todo_id))